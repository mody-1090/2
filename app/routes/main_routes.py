...
from app import db
from app.models import User, WithdrawalRequest, FactoryAgreement, Order, OrderParticipant, Message
...
def customer_order_details(participant_id):
    ...
    return render_template("customer_order_details.html", participant=participant)
...
def dashboard():
    ...
    if user.role == "admin":
        ...
        if request.method == "POST":
            ...
            db.session.commit()
            flash("✅ تم إنشاء الطلبية بنجاح!", "success")
            return redirect(url_for("main_routes.dashboard"))
