...
def upload_file_to_s3(file, intermediary_id):
    """رفع ملف إلى S3 داخل مجلد الوسيط المحدد مع UUID لمنع الاستبدال"""
    file_extension = file.filename.rsplit(".", 1)[1].lower()  # استخراج الامتداد
    unique_filename = f"{uuid.uuid4()}.{file_extension}"  # إنشاء اسم ملف فريد
    folder_name = f"intermediaries/intermediary_{intermediary_id}"  # اسم المجلد داخل S3
    s3_path = f"{folder_name}/{unique_filename}"  # المسار الكامل داخل S3
